...
def sin1_validroots(q, r, chicons, chi1, chi2, k1, k2):
    x1s1=-1 * (k2)**(-1) * ((1 + q))**(-2) * (((1 + q))**(2) * (r)**(1/2) + 
...
def sin2_validroots(q, r, chicons, chi1, chi2, k1, k2):
...
def A_validroots(q, r, chicons, chi1, chi2, k1, k2):
...
def sinDelPhiroots(kappa, u, chicons, q, chi1, chi2, k1, k2, precomputedroots=None):
    """
    ...
    precomputedroots: array, optional (default: None)
        Pre-computed output of Sin(DeltaPhi) roots for computational efficiency.
    ...
    Examples
    --------
    `all_roots = precession_NS.sinDelPhiroots(kappa, u, chicons, q, chi1, chi2, k1, k2, precomputedroots=precomputedroots)`
    """
    r=precession.eval_r(u=u, q=q)
...
def deltachiroots_old(kappa,  u, chicons, q, chi1, chi2, k1, k2, precomputedroots=None):
    """
    Roots of the cubic equation in deltachi that describes the dynamics on the precession timescale.
...
def deltachiroots(kappa,  u, chicons, q, chi1, chi2, k1, k2, precomputedroots=None):
    ...
    if precomputedroots is None:
        ...
        costheta1_x = eval_costheta1(x, chicons, r, q, chi1, chi2, k1, k2)
        costheta2_x = eval_costheta2(x, chicons, r, q, chi2, k1, k2)
        theta1_x = np.arccos(np.clip(costheta1_x, -1, 1))
        theta2_x = np.arccos(np.clip(costheta2_x, -1, 1))
        
        A2 = (eval_A(kappa, q, r, chi1, chi2, theta1_x, theta2_x))**2
        B2 = (eval_B(q, chi1, chi2))**2
        
        y=sin1_2(x, r,  chicons,  q, chi1, k1,k2) * sin2_2(x, r,  chicons,  q, chi2, k1,k2) - (A2/B2)
        #y = dchidt2_RHS(x,kappa,r,chicons,q,chi1,chi2,k1,k2)
        mask = np.isfinite(y)
        spl = CubicSpline(x[mask],y[mask]/np.max(y[mask]) )
        roots= spl.roots()
...
def deltachiroots_combined(kappa, u, chicons, q, chi1, chi2, k1, k2):
    r = precession.eval_r(u=u, q=q)

    #eval roots indiviually 
    s1 = sin1_validroots(q, r, chicons, chi1, chi2, k1, k2)
    s2 = sin2_validroots(q, r, chicons, chi1, chi2, k1, k2)
    A_ = A_validroots(q, r, chicons, chi1, chi2, k1, k2)
    dphi = sinDelPhiroots(kappa, u, chicons, q, chi1, chi2, k1, k2)
...
def eval_A(kappa, q, r, chi1, chi2, theta1, theta2):
    m1 = precession.eval_m1(q)
    m2 = precession.eval_m2(q)
    M = m1 + m2 
    A = 2*kappa*q*((q+1)**2)*((r)**(1/2)) - (M**2)*(2*q*chi1*np.cos(theta1)*(q*chi2*(M**(1/2))*np.cos(theta2) + r**(1/2)) + 2*(q**3)*chi2*(r**(1/2))*np.cos(theta2) + (M**(1/2))*((q**4)*(chi2**2) + (chi1**2)))
    return A 
    
def eval_B(q, chi1, chi2):
    m1 = precession.eval_m1(q)
    m2 = precession.eval_m2(q)
    M = m1 + m2 
    B = 2*(M**(5/2))*(q**2)*chi1*chi2
    return B 
...
def isotropic_masses(N=1):
    ...
    m1=np.random.uniform(1.4,2.17,N)
    m2=np.random.uniform(1.4,2.17,N)

...
\section{New things}
\begin{itemize}
 \item UD instability is not the case for the bifurcation to happen.
 \item Mathematica is super fast in finding the solution and does not require any interpolator. We should use it to find the solutions [Try to implement a Python Mathematica code], otherwise we taylor expand around $\deltachi \sim 0$ and use the solution as an initial guess (however not clear which order we should use)
 \item Maybe the precession period became very large because it has more mountains to climb. Try with a toy problem to test if this is true
